...
class GameView(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    data class Medkit(val id: Int, val x: Float)
    private val medkits = mutableListOf<Medkit>()
    private var medkitSpawnTimer = 0
    private var nextMedkitId = 1
    ...
    private val medkitPaint = Paint().apply { color = Color.parseColor("#7FE89A") }
    private val medkitCrossPaint = Paint().apply { color = Color.WHITE }
    ...
    fun resetGame() {
        ...
        bullets.clear()
        medkits.clear()
        medkitSpawnTimer = 180 // ~3s at 60fps before the first one can appear
    ...
    fun onNetworkMessage(msg: String) {
        ...
        when (parts[0]) {
            ...
            "MEDKIT_SPAWN" -> {
                ...
                if (medkits.none { it.id == id }) {
                    medkits.add(Medkit(id, x))
                }
            }
            "MEDKIT_TAKEN" -> {
                val id = parts[1].toIntOrNull() ?: return
                medkits.removeAll { it.id == id }
            }
        }
    }
    ...
    private fun tick() {
        ...
        // Medkits: only the host decides when a new one spawns (avoids both
        // sides spawning independently and disagreeing on the battlefield).
        if (isHost) {
            medkitSpawnTimer--
            if (medkitSpawnTimer <= 0 && medkits.size < 2) {
                val maxX = (width - SOLDIER_W).coerceAtLeast(1f)
                val x = Random.nextFloat() * maxX
                val id = nextMedkitId++
                medkits.add(Medkit(id, x))
                sendMessage?.invoke("MEDKIT_SPAWN:$id:$x")
                medkitSpawnTimer = 300 + Random.nextInt(300) // ~5-10s
            }
        }

        // Either side can walk over a medkit and heal if below full health.
        val kitIter = medkits.iterator()
        while (kitIter.hasNext()) {
            val kit = kitIter.next()
            val myCenter = myX + SOLDIER_W / 2f
            if (abs(myCenter - kit.x) < 55f && myHp < 100) {
                myHp = (myHp + 20 + Random.nextInt(10)).coerceIn(0, 100)
                pushHud()
                sendMessage?.invoke("MEDKIT_TAKEN:${kit.id}")
                kitIter.remove()
            }
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (kit in medkits) {
            drawMedkit(canvas, kit.x, soldierBottom)
        }
    ...
    private fun drawMedkit(canvas: Canvas, x: Float, bottom: Float) {
        val size = 46f
        val top = bottom - size - 20f
        val rect = RectF(x, top, x + size, top + size)
        canvas.drawRoundRect(rect, 8f, 8f, medkitPaint)
        // white plus sign
        val cx = x + size / 2f
        val cy = top + size / 2f
        canvas.drawRect(cx - 4f, cy - 14f, cx + 4f, cy + 14f, medkitCrossPaint)
        canvas.drawRect(cx - 14f, cy - 4f, cx + 14f, cy + 4f, medkitCrossPaint)
    }
